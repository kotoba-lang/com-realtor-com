(ns realtor_com.main
  "Kotodama WASM entrypoint for the Realtor_com clean-room actor (L4) — Clojure port.
  ...
  facts (`realtor_com.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "realtor_com")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "realtorc_lis"
   ...
   {:entity "Tenant"      :plural "tenants"      :id-prefix "realtorc_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"realtor_com.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "realtor_com-compat" :tier tier :entities entities} 200])
...
(defn start! [] :realtor_com-compat/ready)
